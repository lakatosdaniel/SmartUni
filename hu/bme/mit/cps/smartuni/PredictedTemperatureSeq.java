...
/**
* A sequence of PredictedTemperature instances.
*/
public final class PredictedTemperatureSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public PredictedTemperatureSeq() {
        super(PredictedTemperature.class);
    }

    public PredictedTemperatureSeq (int initialMaximum) {
        super(PredictedTemperature.class, initialMaximum);
    }

    public PredictedTemperatureSeq (Collection<?> elements) {
        super(PredictedTemperature.class, elements);
    }

    public  PredictedTemperature get(int index) { 
        return (PredictedTemperature) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, PredictedTemperature.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' PredictedTemperature objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(PredictedTemperature.create());
